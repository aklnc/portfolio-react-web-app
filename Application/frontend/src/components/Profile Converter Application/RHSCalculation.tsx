...
const RHSCalculation = () => {
  ...
  const [fetchData, setFetchData]: [
    RHSOutput,
    React.Dispatch<React.SetStateAction<RHSOutput>>
  ] = useState({
    g: 0,
    u: 0,
    a: 0,
  ...
  const Calculate = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (+h > 0 && +b > 0 && +t > 0 && +qty > 0 && +length > 0) {
      if (+h <= 2 * +t || +b <= 2 * +t) {
        ...
        axios
          .post(`${process.env.REACT_APP_BACKEND_URL}/builtup-app/rhs`, {
            h: +h,
            b: +b,
            t: +t,
            qty: +qty,
