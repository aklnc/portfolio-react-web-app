...
const LShapedCalculation = () => {
  /*interface LShapedInput {
    h: number;
    b: number;
    t: number;
    qty: number;
  ...
  const [fetchData, setFetchData]: [
    LShapedOutput,
    React.Dispatch<React.SetStateAction<LShapedOutput>>
  ] = useState({
    g: 0,
    u: 0,
    a: 0,
  ...
  const Calculate = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    axios
      .post(`${process.env.REACT_APP_BACKEND_URL}/builtup-app/l-shaped`, {
        h: +h,
        b: +b,
        t: +t,
        qty: +qty,
          ...
          <Table striped className="output-table">
            <tbody>
              <tr className="tr-00">
                ...
                <td>
                  <b>{(+h === +b) ? ("L" + h + "X" + t + '-' + length + ' #' + qty): ("L" + h + "X" + b + "X" + t + '-' + length + ' #' + qty)}</b>
                </td>
                <td>
                  <b>Unit</b>
                </td>
              ...
              <tr className="tr-05">
                <td>Eccentricity (ez):</td>
                <td>{+h - fetchData.zc}</td>
                <td>[mm]</td>
              </tr>
              <tr className="tr-06">
...
export default LShapedCalculation;
