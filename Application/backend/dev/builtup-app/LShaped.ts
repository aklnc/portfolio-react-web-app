...
export default (input: LShapedInput): LShapedOutput => {
  ...
  let yc =
    ((input.t / 2) * (input.b ** 2 + input.h * input.t - input.t ** 2)) / a;
  let zc =
    ((input.t / 2) * (input.h ** 2 + input.b * input.t - input.t ** 2)) / a;

  let iy =
    (input.t * (input.h - zc) ** 3 +
      input.b * zc ** 3 -
      (input.b - input.t) * (zc - input.t) ** 3) /
    3;
  let iz =
    (input.t * (input.b - yc) ** 3 +
      input.h * yc ** 3 -
