...
exports.default = (input) => {
    ...
    let yc = (yc1 * input.t + (yc2 * input.b) / 2 + yc3 * (input.b - input.t / 2)) /
        (yc1 + yc2 + yc3);
    let iy1 = (input.t * h_prime ** 3) / 12;
    let iy2 = (input.b * input.t ** 3) / 12 +
        input.b * input.t * ((input.h - input.t) / 2) ** 2;
    let iy3 = (input.t * c_prime ** 3) / 12 +
        input.t * c_prime * ((input.h - input.c - 3 * input.t) / 2) ** 2;
    let iy = iy1 + 2 * iy2 + 2 * iy3;
    let iz1 = (h_prime * input.t ** 3) / 12 + h_prime * input.t * (yc - input.t / 2) ** 2;
    let iz2 = (input.t * input.b ** 3) / 12 + input.b * input.t * (input.b / 2 - yc) ** 2;
    let iz3 = ((input.c - input.t) * input.t ** 3) / 12 +
